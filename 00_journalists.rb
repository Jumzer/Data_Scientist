...
handle_AZ = journalists.sort_by{|word| word.downcase}
puts handle_AZ

#Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)
handle_size = journalists.sort{|x, y| x.length <=> y.length}
puts handle_size
...
def handle_x (journalists,x)
    who = journalists.select {|handle| handle.length == x+1} 
    how_many = who.count
    puts "Voici le nombre de handle contenant #{x} caractères : #{how_many}"
end

handle_max = journalists.max{|x,y| x.size <=> y.size}


handle_max.size.times do |compteur|
    handle_x(journalists,compteur)
end
